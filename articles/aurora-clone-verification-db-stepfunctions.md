...
3. EventBridge + 別の Step Functions が、**不要になった検証用クローンを毎日削除**
...
実際に回してみて「ここは気をつけたほうがいい」と感じたところと、  
あわせて**どう回避するか / どう考えているか**も書いておきます。

- クローンを長期間放置すると、差分課金がじわじわ効いてくる  
  - → 短命で使い捨てる前提にして、今回のように **タグ + 定期削除（毎日0時）** を組み合わせて「必ずどこかで消える」ようにしておく。  
  - → もっと厳密にやりたければ、`ClusterCreateTime` を見て **「作成から◯時間以上」** を条件に追加するのもあり。

- Describe 系 API はそこまで安くないので、ポーリング間隔を攻めすぎると制限に当たる  
  - → 本記事のワークフローでもやっているように、**Wait ステートを挟んで 15〜60 秒間隔でポーリング**するくらいが妥当。  
  - → もしクラスター数や実行頻度が増えてきたら、**ポーリング間隔をさらに伸ばす / 状態遷移の回数を減らす**方向で調整する。

- Auto Cleaner は `DescribeDBClusters` で全クラスタを見るので、タグの付け方を誤ると事故になる  
  - → 実運用では、**クローン用クラスタ名にプレフィックスを付ける**などして、「名前」と「タグ」の両方で削除対象を絞るのが安全。  
  - → 併せて、**本番クラスタ側には `auto_delete` を付けない**といった運用ルールを決めておくと、うっかり事故の可能性をかなり下げられる。
