...
EC2上でRedashを運用する中で、最も深刻だったのはWorkerプロセスによるメモリ枯渇問題でした。大量データの集計処理や複雑なクエリを実行した際に、Workerプロセスがメモリを消費し尽くし、サーバー全体がダウンするという事象が定期的に発生していました。
